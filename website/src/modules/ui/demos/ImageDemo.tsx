import { Image } from '@resolid/nxt-ui';
import { BlurhashCanvas } from '~/common/components/BlurhashCanvas';
import { DemoExample } from '~/modules/ui/components/DemoExample';

export const Basic = () => {
  return (
    <DemoExample
      preview={() => (
        <div className={'grid grid-cols-4 gap-3'}>
          <Image
            fallback={
              <BlurhashCanvas
                width={181}
                height={120}
                hash={'oJKl?HS}8wIupJR%%LI=i_sot6xY4Tso_N$|IUnmD%n%x]WoRjS5Y6NdIAahxaoI-ns,rrs.j]WV'}
              />
            }
            className={'w-auto'}
            src={'https://nxt.resolid.tech/images/01.jpg'}
          />
          <Image
            fallback={
              <BlurhashCanvas
                width={181}
                height={120}
                hash={'o9Cr_.R%03R.}r=__NxYVtNIJAt6IU%2j]9uoL$%D%X9tQr=RjR+IpR*xt%1RkNHS$n$s9S$ofxE'}
              />
            }
            className={'w-auto rounded'}
            src={'https://nxt.resolid.tech/images/02.jpg'}
          />
          <Image
            fallback={
              <BlurhashCanvas
                width={181}
                height={120}
                hash={'o23b]@?wwEtRsiR$H=DhI9Rij[ac$^%itnV?WFn*RjROkCo#jsf,t3ogV[RiWAfSMwWUo#ofozj['}
              />
            }
            className={'w-auto rounded-lg'}
            src={'https://nxt.resolid.tech/images/03.jpg'}
          />
          <Image
            fallback={
              <BlurhashCanvas
                width={181}
                height={120}
                hash={'o03kjBT|9UR4KtO*33kErVnOs7X2DZm+x+%LrpnB:QrZF{KjOHwMKj%3$$NInmkVXcORNG;3$1K4'}
              />
            }
            className={'w-auto rounded'}
            src={'https://nxt.resolid.tech/images/04.jpg'}
          />
          <Image
            fallback={
              <BlurhashCanvas
                width={181}
                height={120}
                hash={'o7G8=r00x[tS8_D%_19Y9FxZWC%M00%L_49F-;tR00~q?aS6_3D%JA%29F%LIpsl?vNHITD%ROkD'}
              />
            }
            className={'w-auto rounded'}
            src={'https://nxt.resolid.tech/images/05.jpg'}
          />
          <Image
            fallback={
              <BlurhashCanvas
                width={181}
                height={120}
                hash={'o7G8=r00x[tS8_D%_19Y9FxZWC%M00%L_49F-;tR00~q?aS6_3D%JA%29F%LIpsl?vNHITD%ROkD'}
              />
            }
            className={'w-auto rounded blur-sm'}
            src={'https://nxt.resolid.tech/images/05.jpg'}
          />
          <Image
            fallback={
              <BlurhashCanvas
                width={181}
                height={120}
                hash={'o7G8=r00x[tS8_D%_19Y9FxZWC%M00%L_49F-;tR00~q?aS6_3D%JA%29F%LIpsl?vNHITD%ROkD'}
              />
            }
            className={'w-auto rounded sepia'}
            src={'https://nxt.resolid.tech/images/05.jpg'}
          />
          <Image
            fallback={
              <BlurhashCanvas
                width={181}
                height={120}
                hash={'o7G8=r00x[tS8_D%_19Y9FxZWC%M00%L_49F-;tR00~q?aS6_3D%JA%29F%LIpsl?vNHITD%ROkD'}
              />
            }
            className={'w-auto rounded grayscale'}
            src={'https://nxt.resolid.tech/images/05.jpg'}
          />
        </div>
      )}
      snippet={`<div className={'grid grid-cols-4 gap-3'}>
  <Image className={'w-auto'} src={'https://nxt.resolid.tech/images/01.jpg'} />
  <Image className={'w-auto rounded'} src={'https://nxt.resolid.tech/images/02.jpg'} />
  <Image className={'w-auto rounded-lg'} src={'https://nxt.resolid.tech/images/03.jpg'} />
  <Image className={'w-auto rounded'} src={'https://nxt.resolid.tech/images/04.jpg'} />
  <Image className={'w-auto rounded'} src={'https://nxt.resolid.tech/images/05.jpg'} />
  <Image className={'w-auto rounded blur-sm'} src={'https://nxt.resolid.tech/images/05.jpg'} />
  <Image className={'w-auto rounded sepia'} src={'https://nxt.resolid.tech/images/05.jpg'} />
  <Image className={'w-auto rounded grayscale'} src={'https://nxt.resolid.tech/images/05.jpg'} />
</div>`}
    />
  );
};

export const Fallback = () => {
  return (
    <DemoExample
      preview={() => (
        <div className={'grid grid-cols-4 gap-3'}>
          <Image
            className={'w-auto'}
            fallback={'https://img.temp.im/199x132'}
            src={'https://nxt.resolid.tech/images/012.jpg'}
          />
          <Image
            className={'w-auto'}
            fallback={
              <div
                className={
                  'flex h-auto w-auto items-center justify-center border border-gray-200 bg-gray-50 text-gray-400'
                }
              >
                Image load fail
              </div>
            }
            src={'https://nxt.resolid.tech/images/012.jpg'}
          />
          <Image
            className={'w-auto'}
            fallback={
              'data:image/png;base64,'
            }
            src={'https://nxt.resolid.tech/images/012.jpg'}
          />
        </div>
      )}
      snippet={`<div className={'grid grid-cols-4 gap-3'}>
  <Image
    className={'w-auto'}
    fallback={'https://img.temp.im/199x132'}
    src={'https://nxt.resolid.tech/images/012.jpg'}
  />
  <Image
    className={'w-auto'}
    fallback={
      <div className={'flex h-auto w-auto items-center justify-center border border-gray-200 bg-gray-50 text-gray-400'}>
        Image load fail
      </div>
    }
    src={'https://nxt.resolid.tech/images/012.jpg'}
  />
  <Image
    className={'w-auto'}
    fallback={'data:image/png;base64,'}
    src={'https://nxt.resolid.tech/images/012.jpg'}
  />
</div>`}
    />
  );
};

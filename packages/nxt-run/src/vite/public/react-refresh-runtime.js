...
export function validateRefreshBoundaryAndEnqueueUpdate(prevExports, nextExports) {
  if (!predicateOnExport(prevExports, (key) => ignoreExport(key) || !!nextExports[key])) {
  ...
  const allExportsAreComponentsOrUnchanged = predicateOnExport(nextExports, (key, value) => {
    hasExports = true;
    if (ignoreExport(key)) return true;
...
const IGNORE_EXPORTS = ['handle', 'links', 'meta', 'shouldRevalidate', 'unstable_shouldReload'];

const ignoreExport = (moduleExport) => IGNORE_EXPORTS.includes(moduleExport);
